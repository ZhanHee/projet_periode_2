...
export const ChifoumiLoss = () => {
    // const [ChifoumiData, setChifoumiData] = useState<any[]>([]);
    const navigate = useNavigate();
    const [monCoupImage, setmonCoupImage] = useState("chaudron");
    const [otherCoupImage, setOtherCoupImage] = useState("balais");
    ...
    const fetchData = async () => {
        try {
            ...
            setmonCoupImage(data1.imageUrl);       //imageUrl doit etre ../../public/${monCoupImage}.png

            const response2 = await fetch(`ici c'est le chemin de ChifoumiPage avec idEleve de l'autre jouer`);
            const data2 = await response2.json();
            setOtherCoupImage(data2.imageUrl);       //imageUrl doit etre ../../public/${monCoupImage}.png
